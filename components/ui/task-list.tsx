...
const StatusBadge = ({ status }: { status: TaskStatus }) => {
  ...
  const statusClasses = {
    ...
    "In Progress": "bg-yellow-100 text-yellow-800 dark:bg-yellow-900/40 dark:text-yellow-400",
        ...
        <table className="w-full text-sm text-left">
          <motion.thead initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ duration: 0.5 }}>
            <tr className="border-b border-border">
              ...
              <th scope="col" className="p-4 font-medium text-muted-foreground">Assignee</th>
              ...
              <th scope="col" className="p-4 font-medium text-muted-foreground text-right">Actions</th>
            </tr>
          </motion.thead>
            ...
            <AnimatePresence>
              {tasks.map((task, index) => (
                ...
                >
                  ...
                  <td className="p-4 text-right">{/* actions injected by parent */}</td>
                </motion.tr>
              ))}
            </AnimatePresence>
          </motion.tbody>
